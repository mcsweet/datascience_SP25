...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - Random because every house has an equal chance of being selected
    vs. only the first 25 homes to exist on the island. No matter how
    houses are selected by people in the simulation.. you get some
    unbiased 25 houses.
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - Collins, Regan and Wilson
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - Families might live near each other to get support for childcare or
    for ease of seeing each other. Those people often have the same last
    name
...
```
    ...
    ## 1 Regan 0.0172    0.0706  0.155   0.05 percentile

**Observations**:

- What is highest possible prevalence for your chosen name, based on the
  confidence interval you constructed?
  - Based on my confidence interval the highest posttible prevalence for
    the last name “Regan” is 0.155.
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - Earlier we determined that taking the first 25 houses introduces
    bias in the data collection. By randomizing what houses we take the
    data from we are able to have a more accurate view of all the
    houses. So if we use the random sample to make the bootstrap we will
    have a bootstrap estimate based on the truly random data not the
    prevalences we see when we just look at the trends from the first 25
    houses.
