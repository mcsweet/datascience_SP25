...
- Which model is *most accurate*? Which is *least accurate*?
  - The most accurate is the cheat model and the least accurate is the
    nonphysical.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_t is an output variable
  - It is effective because T_norm is calculated directly from it
    (T_norm = (avg_t - T_f )/ T_f)
  - It almost directly determines the target making it a good predictor
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No we would not have it as its an output from the simulation not an
    input
  - avg_T is not a valid predictor in practice
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is metadata variable aka it identifies the simulation run. It is
    just a label/ID not a physical thing.
...
- Which columns are excluded in the model formula above? What categories
  ...
  the model?
  - idx
  - avg_q, avg_t, rms_T, T_norm
- Which inputs are *statistically significant*, according to the model?
  - avg_T, avg_q, rms_T depend on simulation results and including them
    would leak information from the result we are trying to predict
  - IDX is metadata label and not physical so gives us nothing
- What is the regression coefficient for `x`? What about the regression
  coefficient for `T_f`?
  - coeffeicnet for t_f: -0.004783
  - x not in model as was excluded
...
- How do these standard deviations relate to the regression coefficients
  for `x` and `T_f`?
  - as T_f increases by 1, T_norm reduces by about 0.00478. T_f has
    large variation so this effect will be impact in real practice
  - x instn included in this model
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - Variable may have real physical importance but not be statistificall
    significant because of a small sample size or low variability in the
    data
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - The fit_q4 is more accurate than fit_simple because the points are
    closer to the 1:1 ratio line, generally smaller vertical error bars,
    and less spread between lines aka closer predicted and actual T_norm
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*?
  - fit_q4 is more confident as narrower prediction interval and shorted
    vertical error bars
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - Fit_simple model only needs x (aka 1)
  - fit_q4 model used 6 (L, W, U_0, N_p, k_f, T_f)
...
- How much do you trust your model? Why?
  - I somewhat trust the model because it has a solid size training data
    set and good predictors but the prediction interval is wide with
    mean there is significant uncertainty
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - A prediction interval is appropriate because we want to estimate the
    range of possible T_nrom values not just the average. We care about
    individual outcomes not mean behavior.
- What fraction of validation cases lie within the intervals you
  ...
  - In the predicted 80% intervals, 65% of validation cases fall within.
    This suggests that the interval estimates are good enough for
    decision making.
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - I would reccomend a T_norm range of -0.9 to 2.7. This captures about
    80% of expected outcomes.
- Are there any other recommendations you would provide?
  - Constrain more variables if you want less uncertainty
