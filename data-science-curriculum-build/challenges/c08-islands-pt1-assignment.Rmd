...
-   Are there any sources of *real* uncertainty in the percent occupied you calculated?
    -   Islanders can move over time between towns and islands. Because of this there is uncertainty in the percent I calculated. If I calculated the percent 2 weeks from now it could be different as one or 5 families could have moved out of or into Helvig
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   I would not know unless I compared the results to another 25 houses in Helvig. Then I could see if this follows the trends of the town.
    -   I can conclude that it is most likely not representative of all houses. For example households with kids could live close together or those with similar incomes or professions. This would create a sample Representative of only that area not all the trends that could lead to homes being near to each other.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    -   Random because every house has an equal chance of being selected vs. only the first 25 homes to exist on the island. No matter how houses are selected by people in the simulation.. you get some unbiased 25 houses.
...
-   Is there any reason that people with the same last name might tend to *live near each other*?
    -   Families might live near each other to get support for childcare or for ease of seeing each other. Those people often have the same last name
...
-   What is highest possible prevalence for your chosen name, based on the confidence interval you constructed?
    -   Based on my confidence interval the highest posttible prevalence for the last name "Regan" is 0.155.
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   Earlier we determined that taking the first 25 houses introduces bias in the data collection. By randomizing what houses we take the data from we are able to have a more accurate view of all the houses. So if we use the random sample to make the bootstrap we will have a bootstrap estimate based on the truly random data not the prevalences we see when we just look at the trends from the first 25 houses.
