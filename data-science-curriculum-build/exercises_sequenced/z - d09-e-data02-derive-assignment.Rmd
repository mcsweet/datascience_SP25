...
The first one is a function that takes input of the data and what it
wants to filter into the output. The filter is saying I want to filter
so I will take this data and this parameter. The second says hey I have
this data and and filter it by xyz. They do the same thing but take
ownership of the data in different ways.
...
When we dont use group_by it takes the mean of all the prices. When we
use group by we get the colums color and then clarity (which is the
groups it calls) and then we get price last. The price is the mean of
all the diamonds that have the same color and clarity values.
...
### **q4** Comment on why the difference is so large. (X,Y,Z are the dimensions of the diamond)
...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    ...
    cov_depth is about 0.0230057 and cov_diff is about 497.5585.

-   Why is `cov_diff` so large?

    even tho there is not much variance in depth there is a small
    difference between depth and depth computed. The cov is high because
    this difference is so low. So the sd doesnt have to be large to get
    a large cov.
...
-   Ideal diamonds are the most expensive and then fair diamonds. This
    is not was I expected. I assumed price would go up as the cut
    quality increased.
-   Price and cut are not highly correlated. They do not follow the same
    trends. They means they most likely depend on the other variables
    for their price.
